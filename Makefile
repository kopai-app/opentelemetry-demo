...
# Kopai replaces the observability stack; use instead of, not alongside, it.
DOCKER_COMPOSE_FILES_KOPAI=-f compose.kopai.yaml
...
.PHONY: start-kopai
start-kopai:
	$(DOCKER_COMPOSE_CMD) $(DOCKER_COMPOSE_ENV) $(DOCKER_COMPOSE_FILES_FULL) $(DOCKER_COMPOSE_FILES_KOPAI) up --force-recreate --remove-orphans --detach
	@echo ""
...
.PHONY: start-kopai-minimal
start-kopai-minimal:
	$(DOCKER_COMPOSE_CMD) $(DOCKER_COMPOSE_ENV) $(DOCKER_COMPOSE_FILES_CORE) $(DOCKER_COMPOSE_FILES_KOPAI) up --force-recreate --remove-orphans --detach
	@echo ""
	@echo "OpenTelemetry Demo with Kopai is running in minimal mode."
	@echo "Make sure Kopai is running on the host (port 4318): npx @kopai/app start"
	@echo "Go to http://localhost:8080 for the demo UI."
	@echo "Go to http://localhost:8000 for the Kopai dashboard."
